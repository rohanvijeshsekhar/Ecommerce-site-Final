...
Performs periodic background synchronization for active shipments with Delhivery API.
Recovers from missed webhooks, handles exponential retries, and reconciles event timestamps.

Usage:
  python manage.py sync_delhivery_shipments
  python manage.py sync_delhivery_shipments --limit 50
"""
...
from apps.shipping.services import DelhiveryService

logger = logging.getLogger("faazo")


class Command(BaseCommand):
    help = "Periodically syncs active Delhivery shipment statuses and appends tracking events."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f"Starting background Delhivery shipment sync (limit={limit})..."))
        ...
        for shipment in active_shipments:
            try:
                ...
                ShippingAPILog.objects.create(
                    shipment=shipment,
                    endpoint=f"/tracking/{shipment.awb_number}",
                    request_method="GET",
        ...
        self.stdout.write(self.style.SUCCESS(f"Finished Delhivery sync. Successfully synced: {synced_count}, Errors: {error_count}"))
