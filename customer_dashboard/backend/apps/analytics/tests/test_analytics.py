...
class GA4AnalyticsServiceTests(TestCase):
    ...
    def test_get_overview_success_with_comparison(self, mock_get_client, mock_is_configured):
        ...
        # Mock current (7 values) and previous (7 values) = 14 metric values
        # [totalUsers, newUsers, sessions, engagementRate, screenPageViews, userEngagementDuration, activeUsers]
        mock_client.run_report.return_value = MockReportResponse(
            rows=[MockRow(dimension_values=[], metric_values=[
                "1500", "800", "1800", "0.6", "4500", "9000.0", "1400",
                "1000", "500", "1200", "0.5", "3000", "6000.0", "900"
            ])]
        )
        ...
        self.assertEqual(data["metrics"]["new_users"], 800)
