...
export const WishlistPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-left pt-[104px] lg:pt-[144px] pb-16 font-sans select-none">
      <div className="max-w-6xl mx-auto px-3.5 sm:px-6 md:px-12 space-y-4 sm:space-y-8">
        
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 border-b border-slate-200/80 pb-3.5 sm:pb-6">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full bg-rose-50 border border-rose-100 text-rose-600 text-[10px] sm:text-xs font-black uppercase tracking-wider mb-1.5 sm:mb-2">
            ...
            <h1 className="text-xl sm:text-2xl md:text-4xl font-black text-slate-900 tracking-tight font-display">
              My Wishlist
            </h1>
          ...
          <button
            onClick={() => router.push('/')}
            className="self-start sm:self-auto inline-flex items-center gap-1.5 text-[11px] sm:text-xs font-bold text-[#006670] hover:text-[#004e56] transition-colors cursor-pointer"
          >
        ...
        {loading && wishlistItems.length === 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-white rounded-xl sm:rounded-2xl border border-slate-100 p-2.5 sm:p-5 space-y-2.5 sm:space-y-3 animate-pulse">
                <div className="aspect-square bg-slate-100 rounded-lg sm:rounded-xl w-full" />
                <div className="space-y-1 sm:space-y-1.5">
                  <div className="h-2 sm:h-2.5 bg-slate-100 rounded w-1/3" />
                  <div className="h-3 sm:h-3.5 bg-slate-100 rounded w-3/4" />
                ...
                <div className="pt-2 sm:pt-3 border-t border-slate-100 space-y-1.5 sm:space-y-2">
                  <div className="h-6 sm:h-7 bg-slate-100 rounded-lg" />
                  <div className="h-6 sm:h-7 bg-slate-100 rounded-lg" />
                </div>
              </div>
            ))}
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-12 md:p-16 border border-slate-200/80 text-center space-y-3 sm:space-y-4 shadow-2xs">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-rose-50 border border-rose-100 flex items-center justify-center text-rose-500 mx-auto">
              <Heart className="w-6 h-6 sm:w-8 sm:h-8" />
            </div>
            <h2 className="text-base sm:text-lg font-bold text-slate-800">Your Wishlist is Empty</h2>
            ...
            <div className="pt-1.5 sm:pt-2">
              <button
                onClick={() => router.push('/')}
                className="px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl bg-[#006670] hover:bg-[#004e56] text-white font-bold text-[11px] sm:text-xs shadow-md shadow-[#006670]/20 transition-all cursor-pointer"
              >
                Browse Products
              </button>
            </div>
