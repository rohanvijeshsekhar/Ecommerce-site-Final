...
const BestSellers: React.FC<BestSellersProps> = ({ 
  ...
  useEffect(() => {
    ...
    const mapProductToBestSeller = (p: any): ProductItem => {
      ...
      const rawRating = p.average_rating ?? p.avg_rating ?? p.rating;
      const rating = rawRating !== undefined && rawRating !== null ? parseFloat(String(rawRating)) : 0;
      const rawReviews = p.total_reviews ?? p.reviews_count ?? p.review_count ?? p.reviews;
      const reviews = rawReviews !== undefined && rawReviews !== null ? parseInt(String(rawReviews), 10) : 0;
      return {
        id:       p.slug,
        title:    p.name,
    ...
    <>
      ...
      <section 
        ...
        style={{ 
          ...
          backgroundImage: `
            linear-gradient(to bottom, #F8FAFC 0%, #F8FAFC 8%, rgba(248, 250, 252, 0.12) 25%, rgba(248, 250, 252, 0.12) 75%, #F8FAFC 92%, #F8FAFC 100%),
            linear-gradient(rgba(0, 77, 84, 0.15) 1px, transparent 1px),
            linear-gradient(90deg, rgba(0, 77, 84, 0.15) 1px, transparent 1px)
          `,
                  ...
                  <SwiperSlide key={`${prod.id}-${idx}`}>
                    <div className="flex flex-col items-center cursor-pointer group" onClick={() => onProductClick?.(prod.id)}>
                      ...
                      <div className="text-center mt-6 slide-details w-full px-2">
                        ...
                        {/* Ratings */}
                        {prod.reviews && prod.reviews > 0 && prod.rating && prod.rating > 0 ? (
                          <div className="flex items-center justify-center gap-1.5 mt-2">
                            <div className="flex items-center text-amber-400">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  className={`w-3.5 h-3.5 ${i < Math.round(prod.rating!)
                          ...
                          <div className="flex items-center justify-center gap-1.5 mt-2">
                            <div className="flex items-center text-slate-300">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  className="w-3.5 h-3.5 stroke-slate-300 fill-none"
                                />
                              ))}
                            </div>
