...
export default function NotificationCenterPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 pt-[112px] lg:pt-[144px] pb-16">
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-[104px] lg:pt-[144px] pb-16 px-3.5 sm:px-6 md:px-8">
      <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6">
        {/* Top Header Card */}
        <div className="bg-gradient-to-r from-[#006670] to-[#004e56] rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-3.5 sm:gap-4">
          <div>
            <div className="flex items-center gap-2.5 sm:gap-3 mb-1 sm:mb-2">
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/10 rounded-xl sm:rounded-2xl flex items-center justify-center backdrop-blur-md shrink-0">
                <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
              </div>
              <h1 className="text-lg sm:text-xl md:text-2xl font-black">Notification Center</h1>
            </div>
            <p className="text-teal-100 text-[11px] sm:text-xs md:text-sm">
          ...
          <div className="flex items-center gap-2 self-start md:self-auto flex-wrap">
            {unreadCount > 0 && (
              <button
                ...
                className="bg-white/10 hover:bg-white/20 text-white font-bold text-[11px] sm:text-xs px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl transition-all flex items-center gap-1.5 cursor-pointer backdrop-blur-md"
              >
                <CheckCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Mark All Read ({unreadCount})
              </button>
            )}
            {totalCount > 0 && (
              <button
                onClick={handleDeleteAll}
                className="bg-rose-500/20 hover:bg-rose-500/30 text-rose-100 font-bold text-[11px] sm:text-xs px-3 sm:px-4 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl transition-all flex items-center gap-1.5 cursor-pointer backdrop-blur-md"
              >
        ...
        {/* Filters Bar */}
        <div className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 shadow-2xs border border-slate-100 flex flex-wrap items-center justify-between gap-2.5 sm:gap-3">
          {/* Category Tabs */}
          <div className="flex items-center gap-1 overflow-x-auto pb-1 md:pb-0 scrollbar-none w-full md:w-auto">
            {categories.map((cat) => (
              <button
                ...
                className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl text-[10.5px] sm:text-xs font-bold transition-all cursor-pointer shrink-0 whitespace-nowrap ${
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className="bg-slate-50 border border-slate-200 text-slate-700 text-[11px] sm:text-xs font-semibold rounded-lg sm:rounded-xl px-2.5 sm:px-3 py-1 sm:py-1.5 focus:outline-none focus:border-[#006670]"
            >
            ...
            <button
              ...
              title="Refresh Feed"
              className="p-1.5 sm:p-2 text-slate-500 hover:text-[#006670] hover:bg-slate-100 rounded-lg sm:rounded-xl transition-all cursor-pointer"
            >
        ...
        {/* Notifications Feed */}
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xs border border-slate-100 overflow-hidden divide-y divide-slate-100">
          {isLoading ? (
