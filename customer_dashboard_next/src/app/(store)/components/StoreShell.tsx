...
export default function StoreShell({ children }: StoreShellProps) {
  ...
  const isCheckoutOrSuccess = pathname === '/checkout' || pathname === '/order-success';

  return (
    <div className={`relative min-h-screen bg-slate-50 flex flex-col w-full ${isCheckoutOrSuccess ? 'pb-0' : 'pb-[56px] md:pb-0'} ${hasWarning ? 'has-warning-banner' : ''}`}>
      <SvgGradients />
        ...
        <div className={`fixed left-0 right-0 z-40 bg-amber-50 border-b border-amber-100 px-4 py-2.5 text-center text-xs font-bold text-amber-800 flex items-center justify-center gap-2 select-none shadow-xs transition-all duration-300 ${
          ...
          <span>
            {user.dealer_status === 'rejected'
              ? 'Your dealer registration was not approved.'
      ...
      {!isCheckoutOrSuccess && <Footer onLogoClick={() => router.push('/')} />}

      {/* Mobile Bottom Navigation Bar (Hidden on Checkout & Order Success for distraction-free checkout flow) */}
      {!isCheckoutOrSuccess && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200/90 pb-safe md:hidden select-none">
          <div className="flex items-center justify-around h-[52px] px-1">
            ...
            >
              <Home className={`w-[20px] h-[20px] transition-all duration-150 ${pathname === '/' ? 'text-[#006670] stroke-[2.2]' : 'text-slate-400 stroke-[1.6]'}`} />
            ...
            >
              <LayoutGrid className={`w-[20px] h-[20px] transition-all duration-150 ${pathname === '/categories' || pathname.startsWith('/categories/') ? 'text-[#006670] stroke-[2.2]' : 'text-slate-400 stroke-[1.6]'}`} />
            ...
            >
              <Compass className={`w-[20px] h-[20px] transition-all duration-150 ${pathname === '/products' || pathname.startsWith('/products/') ? 'text-[#006670] stroke-[2.2]' : 'text-slate-400 stroke-[1.6]'}`} />
            ...
            >
              <User className={`w-[20px] h-[20px] transition-all duration-150 ${pathname === '/profile' ? 'text-[#006670] stroke-[2.2]' : 'text-slate-400 stroke-[1.6]'}`} />
            ...
            >
              <div className="relative">
                <ShoppingBag className={`w-[20px] h-[20px] transition-all duration-150 ${pathname === '/cart' || pathname === '/checkout' || pathname === '/order-success' ? 'text-[#006670] stroke-[2.2]' : 'text-slate-400 stroke-[1.6]'}`} />
